...
public sealed class RollingFileLogger : IAppLogger
{
    ...
    private void OpenNextFile()
    {
        ...
        do
        {
            path = Path.Combine(
                _directory,
                $"whispdows-{DateTime.UtcNow:yyyyMMdd-HHmmss-fff}-{Environment.ProcessId}-{_fileSequence++:D3}.log");
        }
    ...
    {
        var files = Directory
            .EnumerateFiles(_directory, "whispdows-*.log", SearchOption.TopDirectoryOnly)
